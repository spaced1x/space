...
- New `src/core/execution/paper.server.ts` implementing the existing `VenueAdapter` interface unchanged: prices against the live official order book, simulates acks and fills, writes ordinary order and fill rows so replay, positions, PnL and statistics behave exactly as in V2.
...
- Registry gains per-provider enable/disable, ACTIVE vs STANDBY, and validation before promotion: enabling a provider makes it active only after it produces a valid sample, otherwise the current provider stays active and the attempt is reported. No automatic switching, ever.
